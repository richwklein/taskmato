...
Taskmato uses [Swift Testing](https://developer.apple.com/documentation/testing) (`import Testing`, `@Test` macros). There is no numeric coverage gate. Instead, this charter names the per-domain critical paths that must be covered before shipping a signed artifact, and the areas that are exempt.

The charter is the gate. Reviewers enforce it during PR review. CI reports LCOV coverage for visibility, not as a pass/fail signal. The LCOV pipeline fix in #280 is a hard prerequisite for the coverage reporting this charter relies on.
...
- Aggregation helpers: `focusTotals(by:)`, `focusTotalsByTask(in:)`, `focusTotalsByDay(in:)`, `focusTotalsByProvider(in:)`, `currentStreak(now:)` — all keyed correctly at start-of-day boundaries.
