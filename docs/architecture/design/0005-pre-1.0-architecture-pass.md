...
Proposed 2026-06-05; the roadmap it describes has shipped except for the JSON-repository deletion
([#414](https://github.com/richwklein/taskmato/issues/414)). The registry split's new boundary is
recorded in [ADR-0008](../decisions/0008-split-task-registry.md).

**Archival note (2026-09-05):** the milestone headings and release numbers throughout this document
are the historical schedule at the time of review. They are a record, not instructions. Planning
...
Already-planned 0.7.0 issues (#260, #293, ~~#303~~, ~~#330~~, #341, #370, ~~#383~~, ~~#392~~) run alongside. Issue **#387** moves from 0.9.0 → 0.7.0 and closes with PR 11. **#388** closes as part of PR 12. **#392** keyboard shortcut pass (⌘F search focus, ⌘N new task, ⌘1–3 tab navigation) ✅ this branch.
...
The repository extraction and SwiftData migration (#412, #413) have shipped; only the deletion of
the JSON implementations (#414) is outstanding. Deferred originally so refactor pressure stayed
contained while writable expansion was in flight.
...
| 43 | Delete `JSONSessionRepository` and `JSONLocalTaskRepository` once the SwiftData stores are observed healthy in a released build | [#414](https://github.com/richwklein/taskmato/issues/414) |
...
1. **Boundary ADR** — landed as [ADR-0008](../decisions/0008-split-task-registry.md) when the registry split closed (#404). It records the new `ProviderRegistry + SelectionStore + TaskQueryService + TaskSorter` boundary and refines ADR-0001's description of the managing object; ADR-0001 stays as the record of the protocol hierarchy itself, which is unchanged.
...
- [#414](https://github.com/richwklein/taskmato/issues/414) — Delete JSON repository implementations once the SwiftData stores are observed healthy in a released build; #414 owns the readiness criteria (PR 43).
...
- [Design doc 0006 — Stats view model](0006-stats-view-model.md) — refines and corrects the 0.8.0 portion of this roadmap (PRs 17–25): narrows `SessionRepository` to two methods, absorbs #268 into #401, adds `StatScope.thisMonth` and period navigation, formalises the stats sidebar deferral.

## Related issues

Existing issues this design doc resolves or aligns with:

- **#268** — `SessionStore.focusTotals(by: TaskRef)` aggregation — closed as superseded; all helpers absorbed into #401 / PR 18. See [design doc 0006](0006-stats-view-model.md).
- **#270** — Today / 7-day / month / all-time charts — implemented at PR 19. See [design doc 0006](0006-stats-view-model.md).
- **#271** — Streak in popover footer — implemented at PR 20. See [design doc 0006](0006-stats-view-model.md).
