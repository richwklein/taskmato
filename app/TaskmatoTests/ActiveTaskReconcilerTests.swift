...
struct ActiveTaskReconcilerTests {
  ...
  private func makeSelectionStore() -> ActiveTaskStore {
  ...
  private func makeSUT(
    ...
    ActiveTaskReconciler(
      registry: registry, activeTaskStore: activeTaskStore, engine: engine,
  ...
  @Test func noActiveTaskIsANoOp() async {
    ...
    let activeTaskStore = makeSelectionStore()
    let sut = makeSUT(registry: registry, activeTaskStore: activeTaskStore)
    await sut.reconcile()
  ...
  @Test func doesNotClearBeforeProviderHasLoaded() async {
    ...
    #expect(activeTaskStore.activeTask == task)
  }
  ...
  @Test func reEnablingRestoresActiveTaskOnceReloaded() async {
    ...
    let sut = makeSUT(registry: registry, activeTaskStore: activeTaskStore)
    await sut.reconcile()
    #expect(activeTaskStore.activeTask == task)  // disabled: left untouched
    ...
    #expect(activeTaskStore.activeTask == task)  // task is still there — restored, not cleared
  }
  ...
  @Test func refreshesSnapshotWhenTitleChangedExternally() async {
    ...
    registry.setLists([], forProviderID: "alpha")
    let activeTaskStore = makeSelectionStore()
    activeTaskStore.track(original)

    let sut = makeSUT(registry: registry, activeTaskStore: activeTaskStore)
    await sut.reconcile()

    #expect(activeTaskStore.activeTask?.title == "New title")
  }
  ...
  @Test func refreshingDoesNotFireOnActiveTaskChanged() async {
    ...
    activeTaskStore.track(original)
    var fired = false
    activeTaskStore.onActiveTaskChanged = { _ in fired = true }
  ...
  @Test func idleRemovalClearsSilently() async {
    ...
    registry.setLists([], forProviderID: "alpha")
    let activeTaskStore = makeSelectionStore()
    activeTaskStore.track(makeItem(providerID: "alpha", nativeID: "1", title: "Gone"))
    let errorPresenter = ErrorPresenter()

    let sut = makeSUT(
      registry: registry, activeTaskStore: activeTaskStore, errorPresenter: errorPresenter)
    await sut.reconcile()
  ...
  @Test func midFocusRemovalPresentsWarningBanner() async {
    ...
    let sut = makeSUT(
      registry: registry, activeTaskStore: activeTaskStore, attribution: attribution,
      errorPresenter: errorPresenter)
    await sut.reconcile()
  ...
  @Test func runningFocusRemovalPausesTheTimer() async {
    ...
    let sut = makeSUT(
      registry: registry, activeTaskStore: activeTaskStore, engine: engine, attribution: attribution
    )
    await sut.reconcile()
  ...
  @Test func runningBreakRemovalDoesNotPauseTheTimer() async {
    ...
    let sut = makeSUT(registry: registry, activeTaskStore: activeTaskStore, engine: engine)
    await sut.reconcile()
  ...
  @Test func stagedLegRunsWithNoActiveTaskAtAll() async {
    ...
    let activeTaskStore = makeSelectionStore()
    activeTaskStore.stage(makeItem(providerID: "alpha", nativeID: "1", title: "Gone"))

    let sut = makeSUT(registry: registry, activeTaskStore: activeTaskStore)
    await sut.reconcile()

    #expect(activeTaskStore.stagedTask == nil)
  }
  ...
  @Test func stagedLegRunsWhenStagedProviderDiffersFromActiveProvider() async {
    ...
    let activeTaskStore = makeSelectionStore()
    activeTaskStore.track(makeItem(providerID: "local", nativeID: "1", title: "Active"))
    activeTaskStore.stage(makeItem(providerID: "reminders", nativeID: "1", title: "Gone"))
    ...
    #expect(activeTaskStore.stagedTask == nil)
    #expect(activeTaskStore.activeTask?.title == "Active")
  }
  ...
  @Test func renamedStagedTaskRefreshesInPlace() async {
    ...
    registerAndLoad(provider, in: registry)
    let activeTaskStore = makeSelectionStore()
    activeTaskStore.stage(original)

    let sut = makeSUT(registry: registry, activeTaskStore: activeTaskStore)
    await sut.reconcile()

    #expect(activeTaskStore.stagedTask?.title == "New title")
  }
