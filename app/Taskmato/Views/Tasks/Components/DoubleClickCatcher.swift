...
extension View {
  ...
  /// Wires the nearest `NSTableView`'s native double-click to `action`, letting double-click
  /// activation coexist with the task list's single-click selection.
  ///
  ...
  /// first click of the double-click has already selected the row.
  func onTableDoubleClick(perform action: @escaping () -> Void) -> some View {
    overlay(TableDoubleClickInstaller(action: action).allowsHitTesting(false))
  }
}
...
private struct TableDoubleClickInstaller: NSViewRepresentable {
  ...
  final class Coordinator: NSObject {
    ...
    /// Finds the backing `NSTableView` and wires its double-click action.
    func attach(startingFrom view: NSView) {
      DispatchQueue.main.async { [weak self] in
        guard let self else { return }
        guard let table = findTable(startingFrom: view) else { return }
    ...
    private func findTable(startingFrom view: NSView) -> NSTableView? {
      var candidate: NSView? = view
      while let current = candidate {
        if let table = tableDescendant(in: current) { return table }
        candidate = current.superview
      }
      return nil
    }

    private func tableDescendant(in view: NSView) -> NSTableView? {
      if let table = view as? NSTableView { return table }
      for subview in view.subviews {
        if let table = tableDescendant(in: subview) { return table }
      }
      return nil
    }
